...
if test_type == 'access_time':
  ...
  df_markers = df
  df = df_markers.groupby(np.arange(len(df_markers))//50).mean()
  ...
  plt.plot(df['Iteration'], df['Diff_Inotify'],  's', fillstyle='full', markersize=5, color='gray', label='inotify', linestyle='solid', linewidth=.7)
  plt.plot(df['Iteration'], df['Diff_eBPF'],  'x', markersize=5, color='red', label='Scylla', linestyle='--', linewidth=.7)
...
if test_type == 'overhead':
  ...
  #plt.text(-.92, mean_at-120, f'{round(mean_at)}', color='dimgray', fontsize=10, ha='center')
  plt.yticks(list(range(0,45000,10000))+[mean_at, 45000])
  #plt.text(-.61, mean_at-680, "-", color='black', fontsize=10, ha='center')
  #plt.text(-.62, mean_at-680, "-", color='black', fontsize=10, ha='center')
...
if test_type == 'resource_usage':
  ...
  df_markers = df
  df_markers = df_markers.groupby(np.arange(len(df_markers))//20).mean()
  ...
  plt.plot(df_markers['Time'], df_markers['AT_CPU_USAGE'], '^',  markersize=5, color='black', label='Normal Access', linestyle='solid', linewidth=.5)
  plt.plot(df_markers['Time'], df_markers['INOTIFY_CPU_USAGE'], 's', fillstyle='full', markersize=5, color='gray', label='inotify', linestyle='solid', linewidth=.5)
  plt.plot(df_markers['Time'], df_markers['SCYLLA_CPU_USAGE'], 'x',  markersize=5, color='red', label='Scylla', linestyle='dotted', linewidth=.5)

  plt.xticks(list(range(0,155,25)))
  plt.legend()
  plt.xlabel("Time [s]")
